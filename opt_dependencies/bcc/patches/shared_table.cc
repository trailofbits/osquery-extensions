 ...
 public:
  class iterator : public TableStorageIteratorImpl {
   ...
   public:
    ...
    virtual ~iterator() {}
    virtual unique_ptr<self_type> clone() const override { return std::make_unique<iterator>(it_); }
...
bool SharedTableStorage::Find(const string &name, TableStorage::iterator &result) const {
  ...
  if (it == tables_.end())
    return false;
  result = TableStorage::iterator(std::make_unique<iterator>(it));
  return true;
}
...
unique_ptr<TableStorageIteratorImpl> SharedTableStorage::erase(const TableStorageIteratorImpl &it) {
  ...
  return std::make_unique<iterator>(tables_.erase(i));
}
...
unique_ptr<TableStorage> createSharedTableStorage() {
  auto t = std::make_unique<TableStorage>();
  t->Init(std::make_unique<SharedTableStorage>());
