...
StatusTuple TypeCheck::check_lookup_method(MethodCallExprNode *n) {
  ...
  if (n->block_->scope_) {
    auto result = std::make_unique<StructVariableDeclStmtNode>(table->leaf_id()->copy(), std::make_unique<IdentExprNode>("_result"),
  ...
  if (!n->init_.empty()) {
    ...
    for (auto f = type->stmts_.begin(); f != type->stmts_.end(); ++f) {
      if (used.find((*f)->id_->name_) == used.end()) {
        auto id = std::make_unique<IdentExprNode>(n->id_->name_);
        id->append_dot((*f)->id_->name_);
        n->init_.push_back(std::make_unique<AssignExprNode>(move(id), std::make_unique<IntegerExprNode>("0")));
      }
    }
...
StatusTuple TypeCheck::visit_state_decl_stmt_node(StateDeclStmtNode *n) {
  ...
  if (s1) {
    ...
    auto offset_var = std::make_unique<IntegerVariableDeclStmtNode>(std::make_unique<IdentExprNode>("$" + name), "64");
    offset_var->init_.push_back(std::make_unique<AssignExprNode>(offset_var->id_->copy(), std::make_unique<IntegerExprNode>("0")));
    ...
    auto asn1 = std::make_unique<AssignExprNode>(std::make_unique<IdentExprNode>("$" + n->id_->name_),
                                            std::make_unique<IdentExprNode>("parsed_bytes"));
    n->init_.push_back(std::make_unique<ExprStmtNode>(move(asn1)));
    auto add_expr = std::make_unique<BinopExprNode>(std::make_unique<IdentExprNode>("parsed_bytes"), Tok::TPLUS,
                                               std::make_unique<IntegerExprNode>(std::to_string(p->bit_width_ >> 3), 64));
    auto asn2 = std::make_unique<AssignExprNode>(std::make_unique<IdentExprNode>("parsed_bytes"), move(add_expr));
    n->init_.push_back(std::make_unique<ExprStmtNode>(move(asn2)));
  }
...
StatusTuple TypeCheck::visit(Node *root) {
  ...
  // offset counter
  auto parsed_bytes = std::make_unique<IntegerVariableDeclStmtNode>(
                        std::make_unique<IdentExprNode>("parsed_bytes"), "64");
  parsed_bytes->init_.push_back(std::make_unique<AssignExprNode>(parsed_bytes->id_->copy(), std::make_unique<IntegerExprNode>("0")));
